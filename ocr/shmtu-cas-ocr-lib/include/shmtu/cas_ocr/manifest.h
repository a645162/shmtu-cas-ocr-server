...
// --------------------------------------------------------------------------
// Release-manifest JSON parsing (schema_version 2 only).
//
...
// narrow, schema-aware walker that expects schema_version 2 with the
// structured `models[]` array.  Only v2.x releases are supported.
// --------------------------------------------------------------------------
...
// Expects schema_version 2 with the structured `models[]` array.
// Errors are not thrown — an unparseable / missing input simply yields a
// manifest with schema_version == 0 and empty model lists.  Callers should
// check `manifest.schema_version` and `manifest.model_count` to detect
// failure.
...
// Resolve the asset_stem from a v2 directory layout.  Recognises the
// canonical `mobilenet_v3_small.trislot_decoder.v2_0` stem.  Returns an
// empty string if no candidate is present in `model_dir`.
